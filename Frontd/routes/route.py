...
def view_register_familia():
    try:
        r_familia = requests.get("http://localhost:8090/api/familia/list")
        data_familia = r_familia.json()
        r_generador = requests.get("http://localhost:8090/api/generador/list")
...
def view_edit_person(id):
    ...
    r = requests.get("http://localhost:8090/api/familia/listType")
    lista_tipos = r.json() 


    r1 = requests.get(f"http://localhost:8090/api/familia/get/{id}")    
    ...
    if r1.status_code == 200:
        ...
        if "data" in data_familia and data_familia["data"]:     
            ...
            if familia["tieneGenerador"]:     
                r_generador = requests.get(f"http://localhost:8090/api/generador/get/{familia['id']}")
...
def save_person():
    ...
    r_familia = requests.post("http://localhost:8090/api/familia/save", data=json.dumps(data_familia), headers=headers)     # Hacer la petición para guardar la familia
    
    requests.post("http://localhost:8090/api/generador/save", data=json.dumps(data_generador), headers=headers)    # Hacer la petición para guardar el generador
...
def update_person():
    ...
    r_generador = requests.post("http://localhost:8090/api/generador/update", data=json.dumps(data_generador), headers=headers)

    if r_generador.status_code != 200:
        flash("Error al actualizar el generador: " + r_generador.json().get("data", ""), category='error')
   
    r_familia = requests.post("http://localhost:8090/api/familia/update", data=json.dumps(data_familia), headers=headers)
...
def delete_familia(id):
    ...
    requests.delete(f"http://localhost:8090/api/familia/delete/{id}") 
    requests.delete(f"http://localhost:8090/api/generador/delete/{id}")
...
def get_familia_generador_data():
    ...
    familias = load_data('/Universidad/compu/Ciclo 3/estructura/unidad2/PracticaN3_JoseRiofriosrc/Backend/src/main/java/Data/Familia.json')
    
    generadores = load_data('/Universidad/compu/Ciclo 3/estructura/unidad2/PracticaN3_JoseRiofriosrc/Backend/src/main/java/Data/Generador.json')
    ...
    response = requests.get('http://localhost:8090/api/familia/contadorGeneradores')    
    data = response.json()
...
def buscar_familias():
    ...
    integrantes = request.args.get('integrantes')
    tiene_generador = request.args.get('tieneGenerador')
    ...
    if apellido_paterno:
        params['apellidoPaterno'] = apellido_paterno
    if apellido_materno:
        params['apellidoMaterno'] = apellido_materno
    if integrantes:
        params['integrantes'] = integrantes
    if tiene_generador:
        params['tieneGenerador'] = tiene_generador

    # Hacer la petición al backend
    try:
        response = requests.get('http://localhost:8090/api/familia/buscar', params=params)
...
def ordenar_familias():
    ...
    try:
        response = requests.get(
            'http://localhost:8090/api/familia/ordenar',
